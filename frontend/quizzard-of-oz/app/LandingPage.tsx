...
// 12 question marks spread across screen with 5 different drift paths
const QM_DATA = [
...
// 8 ambient question marks fixed in the viewport, gently hovering
const AMB_QM_DATA = [
...
// 6 cards — negative delays = already mid-flight on load, varied speeds
  ...
  return (
    <>
      <style>{`
        ...
        /* ── ambient hovering question marks (stay in viewport) ── */
        @keyframes ambFloat0 {
      ...
      <div
        ...
        style={{ background: 'linear-gradient(135deg, #0d0221 0%, #1a0545 40%, #0f1e5c 70%, #0d0221 100%)' }}
      >
        ...
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          ...
          {/* 8 ambient question marks — fixed in viewport, gently hovering */}
          ...
          {/* 12 floating question marks — each with its own drift path */}
          {QM_DATA.map((qm, i) => (
            <div
              key={i}
              className="absolute font-bold select-none"
          ...
          {/* 6 floating quiz cards — each with its own drift path */}
          {CARD_DATA.map((item, i) => (
            ...
            >
              <div style={{ fontSize: '0.5rem', color: 'rgba(196, 181, 253, 0.45)', marginBottom: '5px', letterSpacing: '0.12em', textTransform: 'uppercase' }}>
                Frage
              </div>
              <div style={{ fontSize: '0.63rem', color: 'rgba(255, 255, 255, 0.35)', lineHeight: 1.45 }}>
                {item.q}
              </div>
              <div style={{ marginTop: '8px', fontSize: '0.6rem', color: 'rgba(245, 158, 11, 0.38)', fontWeight: 600 }}>
                {item.a}
              </div>
            </div>
          ))}
        ...
        <main className="relative z-10 flex flex-col items-center justify-center min-h-[calc(100vh-73px)] px-4">
          ...
          <div className="flex flex-col items-center gap-4 w-full max-w-sm">
            ...
            {/* Unranked + Übung — kleiner, nebeneinander */}
